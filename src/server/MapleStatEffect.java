    ...
    private static MapleStatEffect loadFromData(final MapleData source, final int sourceid, final boolean skill, final boolean overTime, final byte level) {
        ...
        if (skill) { // hack because we can't get from the datafile...
            switch (sourceid) {
                ...
                case 5121009: //极速领域      
                case 15111005: //极速领域  船长  
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.SPEED_INFUSION, ret.duration *= 1));
                    break;
              /*  case 4321000: //tornado spin uses same buffstats
                ...
                case 22121000:
                    monsterStatus.put(MonsterStatus.FREEZE, ret.duration *= 1);
                    break;
                ...
                case 2221003: // 冰凤球   
                    monsterStatus.put(MonsterStatus.FREEZE, ret.duration *= 1);                    
                    //monsterStatus.put(MonsterStatus.POISON, ret.duration *= 1);               
                    break;                    
                ...
                case 2221002:
                case 2321002:
                    //statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.MANA_REFLECTION, ret.x));
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.POWERGUARD, ret.x/3));
                    break;
            ...
            default:
                if (isMorph() || isPirateMorph()) {
                    ...
                    if (sourceid == 1013 && applyto.getMountId() != 0 ) {
                        ...
                        } else if (sourceid == 10001014 && applyto.getMountId() != 0 ) {//骑士团飞碟
                        mountid = applyto.getMountId();;
                        mountid2 = applyto.getMountId();
                    }
...
//                        applyto.getClient().sendPacket(MaplePacketCreator.giveMount(applyto, mountid, sourceid, stat));
//                        applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.showMonsterRiding(applyto.getId(), stat, mountid, sourceid), false);
//                    } else {
//                        return;
//                    }
//                    normal = false;
                     ...
                     * applyto.getClient().sendPacket(MaplePacketCreator.cancelBuff(null));
                     * applyto.getClient().sendPacket(MaplePacketCreator.giveMount(applyto,ridingLevel,
                     * sourceid, stat));
    ...
    public boolean isDash() {
        return this.sourceid == 5001005 || sourceid == 5110001 || sourceid == 15001003;
    }

...
//        }
        chrs = new ArrayList<>(slots);
        for (int i = 0; i < chrs.size(); i++) {
            chrs.add(new WeakReference<MapleCharacter>(null));
        }
    }
    ...
    public void addVisitor(MapleCharacter visitor) {
        ...
        if (i > 0) {
            ...
            chrs.set(i - 1, new WeakReference<MapleCharacter>(visitor));
            if (!isOwner(visitor)) {
    ...
    public void removeVisitor(MapleCharacter visitor) {
        ...
        if (slot > 0) {
            ...
            chrs.set(slot - 1, new WeakReference<MapleCharacter>(null));
            if (shouldUpdate) {
                update();
            }
        }
    ...
    public void removeAllVisitors(int error, int type) {
        for (int i = 0; i < chrs.size(); i++) {
            ...
            if (visitor != null) {
                ...
                //chrs[i] = new WeakReference<MapleCharacter>(null);
                chrs.set(i, new WeakReference<MapleCharacter>(null));
            }
        }
        update();
    }
    ...
    public List<Pair<Byte, MapleCharacter>> getVisitors() {
        ...
        for (byte i = 0; i < chrs.size(); i++) { //include owner or no
            if (chrs.get(i) != null && chrs.get(i).get() != null) {
                chrz.add(new Pair<Byte, MapleCharacter>((byte) (i + 1), chrs.get(i).get()));
            }
        }
        return chrz;
    }

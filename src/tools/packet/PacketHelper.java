...
//        }
    ...
    public static void addInventoryInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        if (ServerConstants.调试模式) {
            System.out.println("-------背包装备格子总数：" + chr.getInventory(MapleInventoryType.EQUIP).getSlotLimit());
        }
        mplew.write(chr.getInventory(MapleInventoryType.USE).getSlotLimit()); // use slots
        if (ServerConstants.调试模式) {
            System.out.println("-------背包消耗格子总数：" + chr.getInventory(MapleInventoryType.USE).getSlotLimit());
        }
        mplew.write(chr.getInventory(MapleInventoryType.SETUP).getSlotLimit()); // set-up slots
        if (ServerConstants.调试模式) {
            System.out.println("-------背包设置格子总数：" + chr.getInventory(MapleInventoryType.SETUP).getSlotLimit());
        }
        mplew.write(chr.getInventory(MapleInventoryType.ETC).getSlotLimit()); // etc slots
        if (ServerConstants.调试模式) {
            System.out.println("-------背包其他格子总数：" + chr.getInventory(MapleInventoryType.ETC).getSlotLimit());
        }
        mplew.write(chr.getInventory(MapleInventoryType.CASH).getSlotLimit()); // cash slots
        if (ServerConstants.调试模式) {
            System.out.println("-------背包特殊格子总数：" + chr.getInventory(MapleInventoryType.CASH).getSlotLimit());
        }

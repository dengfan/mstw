...
//        c.getSession().write(mplew.getPacket());
    ...
    public static void NPCTalk(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getConversation() != 0) {
            chr.dropMessage(5, "[系统提示] 角色可能进入假死状态（不能使用技能，不能打开菜单），请使用 @k 命令解除异常。");
            return;
        }
    ...
    public static final void Storage(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (mode) {
            case 4: { // 取出物品
                ...
                if (item != null) {
                    if (!MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), item.getOwner())) {
                        ...
                        if (chr.getMeso() >= takeOutHandlingFee) {
                            chr.gainMeso(takeOutHandlingFee * -1, false, true, false);
                            chr.dropMessage("寄存取物服务费：-" + takeOutHandlingFee);
                        }
                    }
            ...
            case 5: { // 存入物品
                ...
                if (chr.getMeso() < 100) {
                    chr.dropMessage(1, "你的金币不足，不能使用此服务。");
                } else {

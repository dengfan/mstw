        ...
        switch (operation) {
                ...
                if (merch) {
                    ...
                    if (pack == null) {
                        ...
                        if (deletePackage(c.getPlayer().getId(), c.getPlayer().getAccountID(), pack.getPackageid())) {
                            FileoutputUtil.logToFile_chr(c.getPlayer(), "log\\雇佣金币领取记录.log", " 领回金币：" + pack.getMesos());
            ...
            case 26: { // 取出物品
                ...
                if (deletePackage(c.getPlayer().getId(), c.getPlayer().getAccountID(), pack.getPackageid())) {
                    ...
                    FileoutputUtil.logToFile_chr(c.getPlayer(), "log\\雇佣领取记录.log", " 领回金币：" + pack.getMesos() + "：领回道具数量 " + pack.getItems().size() + " 道具：" + item_id + " " + item_name);
                    //c.getPlayer().setLastHM(System.currentTimeMillis());
                } else {
                    c.getPlayer().dropMessage(1, "发生未知错误。");
                }

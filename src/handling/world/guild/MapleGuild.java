...
public class MapleGuild implements java.io.Serializable {
    ...
    public static void 人气排行(MapleClient c, int npcid) {
        try {
            ...
            PreparedStatement ps = con.prepareStatement("SELECT `name`, `fame` as data, `level`, `meso` FROM characters where accountid > 0 ORDER BY `fame` DESC LIMIT 100");
    ...
    public static void 任务排行(MapleClient c, int npcid) {
        try {
            Connection con = DatabaseConnection.getConnection();

            PreparedStatement ps = con.prepareStatement("select `name`, count(queststatusid) as data, `level`, `meso` from queststatus, characters where characterid = id and accountid > 0 AND `status` = 2 and quest <= 99999 GROUP BY characterid ORDER BY data DESC LIMIT 100");
            Throwable localThrowable2 = null;
            ResultSet rs;
            try {
                rs = ps.executeQuery();

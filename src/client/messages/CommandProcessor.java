...
public class CommandProcessor {
    ...
    public static boolean processCommand(MapleClient c, String line, CommandType type) {
        if (line.charAt(0) == ServerConstants.PlayerGMRank.NORMAL.getCommandPrefix()) {
            ...
            if (co == null || co.getType() != type) {
                sendDisplayMessage(c, "输入的命令不存在，可以使用 @help 来查看命令列表。", type);
                return true;
            }
            ...
            try {
                ...
                sendDisplayMessage(c, "命令不存在。", type);
                if (c.getPlayer().isGM() || c.getPlayer().getId() == 1) {
                ...
                if (line.charAt(0) == '!') { //GM Commands
                    ...
                    if (c.getPlayer().getGMLevel() >= co.getReqGMLevel()) {
                        ...
                        if (ret > 0 && c.getPlayer() != null) { //incase d/c after command or something
                            ...
                            System.out.println("[ " + c.getPlayer().getName() + " ] 使用了命令：" + line);
                        }
                    } else {
                        sendDisplayMessage(c, "你不能使用此命令。", type);
                    }
                    return true;
                }
            }
    ...
    static {
        ...
        for (Class<?> clasz : CommandFiles) {
            try {
                PlayerGMRank rankNeeded = (PlayerGMRank) clasz.getMethod("getPlayerLevelRequired", new Class[]{}).invoke((Object) null, (Object[]) null);

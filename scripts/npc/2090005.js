...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (cm.getMapId() == 251000000) {
            cm.sendYesNo("怎么样？我从 " + btwmsg + " 再到现在。我的速度很快的吧，如果你想返回 #b" + menu[3] + "#k ，那么我们就立刻出发，不过还是得给我一些辛苦钱，价格是 #b" + cost[3] + " 枫币#k。");
        } else {
            cm.sendSimple("如果想从 " + btwmsg + " 去的话。给我些辛苦钱就送你。我送你比起你走着去快多了。怎么样？\r\n" + display);
        }
    } else if (status == 1) {
        if (selection == 2) {
            cm.sendYesNo("你确定要去 #b" + menu[2] + "#k ？ 那么你要付给我 #b" + cost[2] + " 枫币#k。");
        } else {
            if (cm.getMeso() < cost[selection]) {
                cm.sendNext("滚!你没有足够的枫币");
                cm.dispose();
            } else {
                if (cm.getMapId() == 251000000) {
                    cm.gainMeso( - cost[3]);
